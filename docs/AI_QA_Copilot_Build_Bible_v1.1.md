...
> Note (S8.0): Phase 8 takes the validated MVP (Phases 0–7) to the §24 **Pilot**
> stage — "Team QA intelligence: repo memory + CI + shared history" (§18:
> "auth, billing, teams, RBAC, deployment hardening" → "pilot-ready product").
> Stance: build on the S0.8 baseline (dev user + JWT HS256 + PBKDF2 +
> project-scoped RBAC, §31.3) — this phase *hardens and generalizes* it rather
> than replacing it: multi-user self-service, organization (team) membership,
> plan/quotas/metering over the existing `organizations.plan` column +
> `ai_actions` usage, and production deployment. **Local-first stays (§29):** no
> cloud LLM, no payment processor (plans are admin-assigned; metering and
> enforcement are real), **SSO/OAuth providers deferred to the Enterprise stage
> (§24)** — V1 auth is email/password + JWT. Reused seams: project-scoped RBAC
> (`require_role`) · `ai_actions` audit + usage (§31.5) · `organizations` /
> `project_members` (§10) · 202 + SSE + `JobAgent` (§31.2) · gitleaks
> fail-closed (§31.4) · S6.5/S7.5 live-E2E + baseline-report pattern. Every
> security-relevant action lands in an audit trail (§17).

| Step | Work | Exit criterion |
|---|---|---|
| S8.1 | Auth hardening — user self-service: `POST /api/v1/auth/register` (email + password policy; duplicate → 409; signup = account + workspace: creates the user **and** their organization, user becomes org owner) · `POST /api/v1/auth/refresh` (opaque rotating refresh token, stored **hashed** in new `user_refresh_tokens` table + migration; reuse-after-rotation rejected) · `POST /api/v1/auth/change-password` (re-auth current password; revokes all refresh tokens) · `GET /api/v1/auth/me` (profile + orgs + projects) · login brute-force throttle (per-email + per-IP, Redis-backed, 429 + `Retry-After`) · password never logged/audited (§17) | register → login → refresh → change-password → re-login all green in tests; rotated refresh token rejected on reuse; throttle trips (429 + `Retry-After`); red-team: no password/refresh-token leak in logs/audit; gates green |
| S8.2 | Teams — organization membership: `organization_members` table (org, user, role `owner`/`member`; one owner per org) + migration · `organization_invites` (email, role, one-time code, 7-day expiry) — code-based, no SMTP (local-first) · routes: `GET /api/v1/organizations` (mine) · `GET/POST/PATCH/DELETE /organizations/{id}/members` (org owner) · `POST /organizations/{id}/invites` + `POST /invites/{code}/accept` (any authenticated user joins as invited) · access model: org membership = baseline access to all org projects (org role maps to a project role when no explicit `project_members` row exists; an explicit row always wins) · non-owner may leave the org | two users + one org: both see shared projects; org outsider → 403 (never 404); invite flow green (accept → member; expiry/reuse rejected); explicit `project_members` row overrides org baseline; gates green |
| S8.3 | RBAC hardening + audit trail: route-level role audit — a test asserts **every** route declares a minimum role (org- or project-scoped) or is explicitly public (login/register/invite-accept/health/webhook) · org-level gates: membership + billing ops = org owner · org deletion = owner with re-auth · `audit_log` table (actor, action, target, outcome, ip, at; append-only) + migration — security events: auth success/failure, membership changes, plan changes, quota denials, deletions (complements `ai_actions`, which is AI-only, §31.5) · deletion workflows (§17): `DELETE /organizations/{id}` (owner; cascades) + user self-delete (purges PII, keeps audit rows) | route-matrix test green (no unguarded route); cross-org access → 403 never 404; audit rows exist for the red-team scenario list (login fail, membership change, plan change, quota denial, org delete); org-delete cascade verified; gates green |
| S8.4 | Billing core (local-first) — plans, quotas, metering: code-defined plan catalog (`free`/`pro`/`enterprise`: max projects, runs/month, AI tokens/month, concurrent jobs) over the existing `organizations.plan` column (default `dev` = free-equivalent) · metering from real usage — `GET /organizations/{id}/usage` (runs from `test_runs`, tokens from `ai_actions` usage, per month) + `GET /organizations/{id}/plan` · enforcement at job dispatch: quota exceeded → 409 `plan_limit` (deterministic, LLM-free, audited in `audit_log`, `job.rejected` SSE for in-flight clients) · plan assignment `PATCH /organizations/{id}` (org owner; local admin can set any plan — no payment processor, §25/§29) | seeded usage → `/usage` numbers exact; quota-exceeded job rejected with clean 409 + audit row + no partial state; plan bump lifts the cap (same job then runs); cross-org usage reads → 403; gates green |
| S8.5 | Deployment hardening: `docker-compose.prod.yml` (API non-root + healthcheck + resource limits + restart policy; DB/Redis unchanged) · reverse-proxy TLS snippet (Caddy) in `infra/` · security middleware: CORS locked to the web origin, security headers (CSP/HSTS/X-Content-Type-Options/Referrer-Policy), request-id, per-user + per-IP rate limiting (Redis, 429 + `Retry-After`) · structured JSON logs with request-id correlation (extend the existing logging config) · secrets via env/secret-manager only, gitleaks fail-closed (§31.4) · backup/restore: `scripts/backup.sh` (pg_dump + artifacts dir, gzip) + `scripts/restore.sh` with a restore round-trip test | `docker compose -f docker-compose.prod.yml up` boots and `/health` is green; header + CORS + rate-limit tests green (429 + `Retry-After`); restore round-trip test recovers a snapshot; no secrets in any committed file (gitleaks); gates green |
| S8.6 | Pilot E2E + baseline report: live pilot scenario — two users (org owner + member) on one org; shared project; full pipeline (requirement → test cases → automation → run → failure → diagnosis → Jira link, the S7.5 loop) as the member · quota denial + plan bump (S8.4) · audit-trail export (S8.3) · live driver committed (evidence pair, S6.5/S7.5 pattern) + baseline `reports/commercialization_v1.json` | live E2E green (all legs, both roles); baseline report committed (single tracked report, `.gitignore` exception per S6.5; drift tracking §31.6/§31.7); all red-team checks pass; gates green (ruff + mypy strict + pytest) |
