...
(signals, evidence, HTTP status, selector, endpoint), the **read-only
source of the application under test** (test-ids, routes, DOM, API shapes,
seed data), and the **actual broken test file** — the ground truth your
patch must be derived from. Re-derive the fix from the code and the
evidence; when the diagnosis's suggested fix conflicts with them, the
code and evidence win.
...
Read-only application context (the app under test — test-ids, routes,
DOM, API shapes, seed data. Reference material only: your patch still
touches only the target test file):
{{app_context}}
...
Ground the fix in the app source (the read-only context above):
- Test-ids are the source of truth for current `data-testid` values.
  Where the test-id module defines **alternate sets** (a base set and a
  defect-flag "drifted" set), use the drifted value ONLY when the
  diagnosis points at locator drift — or the failure shows a test-id
  missing at runtime while the drifted set defines one. Otherwise keep
  the base value.
- Use the router and page components for the real post-action URLs (where
  login lands, where checkout redirects), for headings/roles/names/
  classes, and for which elements exist only after an action (e.g. a cart
  total renders only once the cart has items).
- Use the route handlers for the real endpoints, methods, auth (bearer
  token from `localStorage`), and response shapes, and the seed data for
  real ids and values. Provision missing data through the app's real API
  — never invent an endpoint that is not in the source.
- Conflict rule: the app source is the app as BUILT; the failure text is
  the app as RUNNING (defect flags change runtime behavior). When they
  conflict: runtime evidence wins for what to expect, the app source wins
  for what exists.
