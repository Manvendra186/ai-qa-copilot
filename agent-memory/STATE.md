...
- **Phase:** 0 — Foundation
- **Step:** S0.2 in progress — `docker-compose.yml` committed; bring-up pending Docker Desktop install
- **In-progress note:** infra choice = **Option A (Docker Desktop)** recorded 2026-08-26; user is installing
...
1. Check `docker --version`. If absent: user still installing Docker Desktop (Option A) —
   wait / ask; WSL2 not yet installed (Docker Desktop installer offers it, or `wsl --install`).
2. Port 5432 conflict check: Windows-native PG16 service `postgresql-x64-16` is running on 5432.
   Stop it (`Stop-Service postgresql-x64-16`) — or set `POSTGRES_PORT=5433` in `.env` and
   align DATABASE_URL — before bringing compose up.
3. `docker compose up -d`, wait for healthy, then verify **exit criterion**:
   `docker compose exec db psql -U qa -d qa_copilot -c 'SELECT 1'` → `1`
   `docker compose exec redis redis-cli ping` → `PONG`
4. Commit `step S0.2: compose infra up (pgvector/pg16 + redis7), exit criterion verified`,
...
- **Docker: NOT installed — Option A (Docker Desktop) chosen 2026-08-26; user installing**
...
- **WSL: NOT installed** (Docker Desktop backend needs it; `wsl --install` available)
...
- **S0.2 bring-up:** port 5432 held by Windows-native PG16 service — stop it (or
  `POSTGRES_PORT=5433` in `.env` + update DATABASE_URL) before `docker compose up -d`.
